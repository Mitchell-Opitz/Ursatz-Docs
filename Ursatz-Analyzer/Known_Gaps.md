...
- **`analysis_report.c` is a 13-line file** containing only `#include`s and the
  `analysis_report_destroy` free function, since the rest of `AnalysisReport`'s lifecycle lives
  in `analysis_pipeline.c`. This isn't a bug, just a thin translation unit mirroring the
  `.h`/`.c` convention used elsewhere.
...
- **`segment_by_onset` requires exact onset-time equality** (`onset_equals` in
  ...
  merged.
- **No SMPTE-division MIDI support.** This is delegated to/limited by Ursatz's importer
