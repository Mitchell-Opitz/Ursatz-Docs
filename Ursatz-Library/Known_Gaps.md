...
`cadence_classification.c`, or `motif_detection.c`. Consequence, unchanged from before these
...
(direction/duration-only) data — none of them compute real values from raw MIDI pitch data.
...
Retire that table in favor of a real repository once/if one is built. Same applies to
...
current phase, not a real discrepancy. (2) A "Reference-type Registry synchronization gap"
note describing `VoiceReference`/`RelationshipReference`/`ProvenanceReference` as "proposed
only, blocked" — this is stale; those three were Registry-authorized and implemented well
...
- **Chord-region matching exact-match rigidity** — extends to seventh-chord matching too
  (inherited by construction); no tolerance for extra non-chord tones. Partially mitigated
  by the new beat-grid Sonority construction (see above) but not eliminated.
- **Cadence Classification's PAC vs. IAC** — requires soprano-resolution data that doesn't
  exist anywhere in the codebase; reports "authentic (unclassified)" rather than guessing.
...
- **Motif Detection** — exact repetition and transposition only (transposition-invariant by
  ...
  deferred to v2, to be pulled forward only if the fixed test corpus requires it.
- **Fragmentation transform is not reusable for detection/matching** — confirmed during
  Sentence Detection's development; it only clones caller-supplied event ids, no comparison
